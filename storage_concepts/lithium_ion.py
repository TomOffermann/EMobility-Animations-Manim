...
def accessories() -> Mobject:
    ...
    separator = DashedVMobject(Line(start=[0, -3.5, 0], end=[0, 2.5, 0], stroke_width=7, color=GREEN), num_dashes=10)
    ...
    return VGroup(*power, *electric, separator)


class LithiumBattery(Scene):
    ...
    def construct(self):
        lithium_ions = VGroup(*self.lithium_ions)
        lithium_ions_next = VGroup(*self.lithium_ions_next)
        lithium_ions_return = VGroup(*self.lithium_ions_return)
        electrons = VGroup(*self.electrons)
        rect = Rectangle(BLUE_A,6.2,11).set_fill(BLUE, 0.1).set_y(-0.5).set_z_index(-10)
        self.play(Create(accessories()))
        self.play(Create(metal_oxid(4, 5, 1).set_x(-3).set_y(-0.5)))
        self.play(Create(carbon_grid(4).set_y(-0.5).set_x(3)))
        self.wait(2)
        self.play(Create(lithium_ions))
        self.wait(2)
        self.play(Create(electrons))
        self.wait(2)
        self.play(*self.circle_anim)
        self.wait(2)
        self.animate_loading(electrons, lithium_ions, lithium_ions_next, False)
        self.wait(4)
        self.animate_using(electrons,self.electrons_pos,lithium_ions_next, lithium_ions_return, False)
        self.wait(2)
        self.play(Create(rect))
        self.wait(2)
        self.play(ApplyWave(rect, amplitude=0.15, time_width=10, run_time=2.5, ripples=8, rate_func = linear))
        self.wait(4)
